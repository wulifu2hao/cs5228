...
if __name__ == "__main__":
	if len(sys.argv) >= 3:
		...
		if not isdir(working_dir):
			...
			MAX_IAMGES_NUM_FOR_VOCAB = 1000
			detector = cv2.SIFT(nfeatures=1000)
			NUM_OF_WORD_FOR_VOCAB = 200
			...
			if not isfile(join(result_dir, "vocab.mat")):
				...
				descrs_for_vocab = np.concatenate(descrs_for_vocab, axis=0)
			...
			if not isfile(join(result_dir, "query_bow.mat")):
				print "computing bag of word representation for query images. This may take a while, but the result will be saved for future usage"
				query_image_path = [join(query_dir,f) for f in listdir(query_dir) if isfile(join(query_dir,f)) and splitext(f)[1].lower() in acceptable_image_formats]
